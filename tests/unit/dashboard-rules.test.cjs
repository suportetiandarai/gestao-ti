...
test('técnico atual, último atualizador e técnico removido não substituem o autor da solução', () => {
  const rows = [
    ticket({
      id: 20,
      status: 'Solucionado',
      technician: 'Jorge Luis Macedo Peixoto',
      technicianId: 43,
      solvedAt: '2026-07-23T09:00:00-03:00',
...
test('Jorge Peixoto só aparece quando é autor real e o par chamado/técnico é único', () => {
  const solvedByJorge = ticket({
    id: 30,
    status: 'Solucionado',
    technician: 'Outra pessoa',
    technicianId: 99,
    solutionTechnician: 'Jorge Luis Macedo Peixoto',
    solutionTechnicianId: 43,
    solvedAt: '2026-07-23T10:00:00-03:00',
  });
  const rows = [
    solvedByJorge,
  ...
  assert.deepEqual(core.technicianResolutionsInShift(rows, NOW, 1), [
    { label: 'Jorge Luis Macedo Peixoto', value: 1 },
  ]);
});
