...
test('snapshot altera somente o status atribuído sem técnico individual', () => {
  assert.equal(dashboardTicketStatus(ticket(20, {
    technician_id: null,
    technician_name: null,
  }), reference), 'Aguardando Atribuição');
  assert.equal(dashboardTicketStatus(ticket(21), reference), 'Atribuído');
  assert.equal(dashboardTicketStatus(ticket(22, {
    status_id: 4,
    status: 'Pendente',
    technician_id: null,
    technician_name: null,
