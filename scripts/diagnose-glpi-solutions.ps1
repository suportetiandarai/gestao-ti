...
try {
    ...
    foreach ($id in $ticketIds) {
        ...
        $relations = @(Invoke-GlpiRead -Path "Ticket/$id/Ticket_User")
        $solutions = @(Invoke-GlpiRead -Path "Ticket/$id/ITILSolution")
        $groups = @(Invoke-GlpiRead -Path "Ticket/$id/Group_Ticket")
        $logs = @(Invoke-GlpiRead -Path "Ticket/$id/Log?range=0-199")
        ...
        $technicalGroups = @($groups | Where-Object { [int]$_.type -eq 2 } | ForEach-Object {
            [ordered]@{
                groupId = [int]$_.groups_id
                relationId = [int]$_.id
            }
        })
        ...
        $result.tickets += [ordered]@{
            ...
            assignedAt = [string]$ticket.date_assign
